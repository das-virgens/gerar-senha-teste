...
if (isset($_POST['senha']) && isset($_POST['coordenador'])) {
    ...
    $stmt_user_check = $conn->prepare("SELECT * FROM usuarios WHERE username = ?");
    $stmt_user_check->bind_param("s", $coordenador);
    $stmt_user_check->execute();
    $result_user_check = $stmt_user_check->get_result();

    if ($result_user_check->num_rows > 0) {
        $stmt_user_update = $conn->prepare("UPDATE usuarios SET contador = ? WHERE username = ?");
        $stmt_user_update->bind_param("ss", $numeroSenha, $coordenador);

        if ($stmt_user_update->execute()) {
            echo "Senha atualizada com sucesso na tabela usuarios!";
        } else {
            echo "Erro ao atualizar senha na tabela usuarios: " . $stmt_user_update->error;
        }
        $stmt_user_update->close();
    } else {
        echo "Erro: Usuário não encontrado na tabela usuarios.";
    }
